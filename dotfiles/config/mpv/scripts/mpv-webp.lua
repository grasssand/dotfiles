...
function make_webp_internal(burn_subtitles)
    ...
    args = string.format('ffmpeg -ss %s %s -t %s -i "%s" -lavfi "%s" -loop 0 -lossless "%s" -q:v "%s" -compression_level "%s" -y "%s"', position, copyts, duration, esc(pathname), esc(trim_filters), options.lossless, options.quality, options.compression_level, esc(webpname))
    os.execute(args)
